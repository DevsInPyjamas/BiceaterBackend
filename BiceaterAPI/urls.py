...
urlpatterns = [
    ...
    url(r'^users/(?P<user_id>[0-9]+)/comments/taking=(?P<taking>[0-9]+)&page=(?P<page>[0-9]+)$',
        views.comments_by_user_id, name='Comments by user id'),
    ...
    url(r'^stations/(?P<station_id>[0-9]+)/comments/taking=(?P<taking>[0-9]+)&page=(?P<page>[0-9]+)$',
        views.comments_list, name='Comments by station'),
